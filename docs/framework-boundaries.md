...
MyBatis-Plus adapter 不进入 `jfoundry-spring-boot-starter`。如果业务使用 Spring Data / JPA，应依赖基础 Spring Boot starter，并通过业务侧或后续扩展模块提供 repository、`OutboxMessageStore`、`InboxMessageStore` 实现。
