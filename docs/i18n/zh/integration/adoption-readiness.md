...
采用范围和前置条件，不是通用的生产就绪认证。评估日期为 2026-07-14。
...
| 组合工作流 | 已证明可以从需求、建模一直支撑到实现和端到端验收 | 优先用于 Java 21、Spring Boot、MyBatis-Plus、PostgreSQL、Kafka 和 Redis 项目 |
...
- MyBatis-Plus 与 PostgreSQL 持久化。
- Kafka 集成、Redis/Redisson 分布式锁、事务性 Outbox 和消费端 Inbox。
- DDD 建模、Hexagonal Architecture，以及不使用 Event Sourcing 的按需 CQRS。
...
套件，用来系统验证 Agent 在正确和错误输入下的决策。费用报销 Demo 为一条完整路径提供了较强集成
证据，但不能替代插件的完整行为基准。
...
DDD 和 Hexagonal Architecture 并具备可靠消息链路的业务项目。插件已经可以作为真实项目的架构
治理工具；jfoundry 已达到受控真实项目可用，但在把它作为组织级默认生产框架之前，仍需要稳定的
不可变版本和具体项目的非功能验证。

稳定版本发布、下游升级验证、新运行时或新架构风格获得同等级端到端证据，或者生产准入条件发生
实质变化时，应同步更新本文评估。
