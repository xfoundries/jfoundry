...
persistence unit。Helidon runtime 提供事务绑定的持久化上下文，并采用相同的已识别 Hibernate 可用性失败转换。
显式依赖组合和 Native Image 限制见 [Helidon MP](helidon.md)。

### 直接装配 JPA 或 Hibernate

JPA adapter 是运行时无关的，但不是开箱即用的 raw-Hibernate bootstrap。Spring Boot、受支持的 Quarkus 和 Helidon
...
`OutboxMessageStore` 优先。此项仅为存储装配，不会加入 dispatcher、scheduler、serializer、自动事件外部化或 Inbox
支持。依赖配置和 Native Image 验证见 [Quarkus](quarkus.md)。

Quarkus 的 Outbox 派发加入 `jfoundry-outbox-quarkus-runtime`，并提供真实的 CDI `MessageSender`。
只有设置 `jfoundry.outbox.dispatcher.enabled=true` 后调度才会启用；它不会引入 broker client 或 fallback sender。
...
`InboxMessageStore` 和 `InboxTemplate` Bean。默认 claim strategy 只支持 PostgreSQL 与 MySQL；其他数据库应声明
`JpaInboxClaimStrategy`。应用提供的 `JpaInboxClaimStrategy`、`InboxMessageStore` 或 `InboxTemplate` 优先。
此项仅为存储装配，不会加入 dispatcher、scheduler、serializer、自动事件外部化或 starter。依赖配置和 Native Image
...
`jfoundry-outbox-helidon-runtime`。CDI alternative 的替换规则与 Native Image 范围见 [Helidon MP](helidon.md)。
...
JPA Inbox 内置原子 claim 策略只支持 PostgreSQL 和 MySQL。其他数据库产品需要提供 `JpaInboxClaimStrategy`。当数据库产品未知且未提供策略时，运行时默认实现会快速失败，而不是选择通用方言行为。用户提供的 `InboxMessageStore`、`OutboxMessageStore`、`InboxTemplate` 或 `JpaInboxClaimStrategy` 优先。
