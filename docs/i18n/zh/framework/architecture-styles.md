...
对应的命令 Handler 实现该契约。命令是输入模型，Handler 是应用层实现。只对静态已知 Handler
逐个转发、带重载 `dispatch(...)` 方法的 `*CommandDispatcher`，既没有路由能力，也没有业务编排价值。
只有确实需要通过共享管线在运行时路由命令时，例如可插拔 Handler 注册或通用消息传输，才使用
Dispatcher 或 Command Bus。
...
Onion 的 CQRS 命令使用按业务动作命名的应用 Handler 或 Service，由外环 web、messaging 或
scheduler 调用该应用边界。CQRS 不会给 Onion 引入 Hexagonal 的 `port.in`、`@PrimaryPort` 或
`*UseCase` 要求。Dispatcher 同样需要真实的运行时路由理由；固定转发没有额外价值。
