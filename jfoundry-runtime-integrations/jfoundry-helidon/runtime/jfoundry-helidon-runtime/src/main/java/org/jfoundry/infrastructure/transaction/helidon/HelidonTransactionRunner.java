...
public class HelidonTransactionRunner implements TransactionRunner {
    ...
    public <T> T call(TransactionOptions options, TransactionCallback<T> callback) throws Exception {
        ...
        return switch (options.propagation()) {
            case REQUIRED -> isTransactionActive() ? callInExistingTransaction(callback) : callInNewTransaction(options, callback);
            case REQUIRES_NEW -> callSuspendingExisting(() -> callInNewTransaction(options, callback));
            case SUPPORTS -> isTransactionActive() ? callInExistingTransaction(callback) : callback.execute();
    ...
    private <T> T callInNewTransaction(TransactionOptions options, TransactionCallback<T> callback) throws Exception {
        ...
        try {
            ...
            if (transactionStarted) rollbackIfActive();
            throw exception;
        } catch (Error error) {
            if (transactionStarted) rollbackIfActive();
            throw error;
        } finally {
            if (timeoutConfigured) transactionManager.setTransactionTimeout(0);
        }
    }
    ...
    private <T> T callInMandatoryTransaction(TransactionCallback<T> callback) throws Exception {
        if (!isTransactionActive()) throw new IllegalStateException("Transaction propagation MANDATORY requires an active transaction");
    ...
    private <T> T callSuspendingExisting(TransactionCallback<T> callback) throws Exception {
        if (!isTransactionActive()) return callback.execute();
    ...
    private <T> T callWithoutTransaction(TransactionCallback<T> callback) throws Exception {
        if (isTransactionActive()) throw new IllegalStateException("Transaction propagation NEVER does not allow an active transaction");
        return callback.execute();
    }
    ...
    private void rollbackIfActive() throws Exception {
        if (isTransactionActive()) transactionManager.rollback();
    }
}
