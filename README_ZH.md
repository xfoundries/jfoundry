...
| 运行时集成 | Spring Framework 与 Spring Boot 装配；Quarkus CDI、Jakarta Transactions 与显式 Outbox 外部化装配 |
...
- **Quarkus**：通过 [Quarkus 运行时集成](docs/i18n/zh/implementations/quarkus.md) 接入 CDI 事务、领域事件分发、显式 Outbox 外部化并验证 Native Image。
