...
```text
运行时集成 -> infrastructure -> application -> domain
```
...
持久化、Outbox、Inbox、broker starter 应按业务需要显式引入，不建议一次性加入所有 starter。按模块选择依赖的详细说明见 [业务项目接入指南](docs/getting-started-for-business-projects.md)。
...
Outbox 是可选能力。只有当事件需要跨进程投递、失败重试或可靠外部化时才需要启用。详细说明见 [Transactional Outbox](docs/transactional-outbox.md)。
...
- [持久化 DataConverter 与 MapStruct 使用指南](docs/persistence-data-converters.md)
- [值对象规范](docs/value-object.md)
- [框架边界设计](docs/framework-boundaries.md)
