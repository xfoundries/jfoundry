...
框架内置事务性 Outbox、消费端 Inbox、应用层事务边界、消息发送 SPI、payload 序列化 SPI、MyBatis-Plus 持久化适配器和可复用架构测试规则。
...
| 持久化 | 框架无关持久化契约和 MyBatis-Plus 适配器 |
...
持久化、Outbox、Inbox、broker starter 应按业务需要显式引入，不建议一次性加入所有 starter。按模块选择依赖的详细说明见 [业务项目接入指南](docs/zh/integration/getting-started-for-business-projects.md)。
...
Outbox 是可选能力。只有当事件需要跨进程投递、失败重试或可靠外部化时才需要启用。详细说明见 [Transactional Outbox](docs/zh/integration/transactional-outbox.md)。
...
- 框架语义：[架构风格指南](docs/zh/framework/architecture-styles.md)、[ArchUnit 架构规则](docs/zh/framework/archunit-rules.md)、[框架边界设计](docs/zh/framework/framework-boundaries.md)
- 建模约定：[值对象规范](docs/zh/modeling/value-object.md)、[Repository 与读侧端口迁移指南](docs/zh/modeling/repository-vs-read-ports.md)
- 技术集成：[业务项目接入指南](docs/zh/integration/getting-started-for-business-projects.md)、[持久化 DataConverter 与 MapStruct 使用指南](docs/zh/integration/persistence-data-converters.md)、[Transactional Outbox 事务性发件箱](docs/zh/integration/transactional-outbox.md)
